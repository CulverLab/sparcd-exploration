 ...
 */
const rowKey = (kind: Kind, entry: Entry) => `${idOf(kind, entry)}\u0000${nameOf(kind, entry)}`
...
export function checklistRows(kind: Kind, used: unknown[], shared: unknown[]): ChecklistRow[] {
  ...
  for (const entry of sharedEntries) {
    // The shared list can hold the same record twice. One row for it, or the
    // copy in use matches neither and ticking adds a third.
    const key = rowKey(kind, entry)
    if (seen.has(key)) continue
    seen.add(key)
    const usedIndex = usedEntries.findIndex(
      (candidate, index) =>
        !claimed.has(index) &&
        (rowKey(kind, candidate) === key || matchShared(kind, candidate, sharedEntries) === entry),
    )
