...
import type { DistanceUnit } from '../store';
 ...
 *  (mirrors the uploader's `metersToFeet` in `lib/coords.ts`). */
 ...
 * id; arrow keys move the highlight, Enter selects, Esc closes. Ported from
 * the uploader's `DeploymentPicker` (issue #279: match the uploader — no
 * create-new-location capability here either, only the shared registry).
 */
export function LocationPicker({ locations, value, onChange, distanceUnit = 'meters' }: Props) {
      ...
      <dl className="mt-1 space-y-0.5 font-mono text-[11px] text-inkMute">
        <div className="flex gap-2">
          ...
          <dd className="truncate">
            <span className={distanceUnit === 'meters' ? 'text-ink' : ''}>{loc.elevation} m</span>
            {' · '}
            <span className={distanceUnit === 'feet' ? 'text-ink' : ''}>
    ...
    <div ref={rootRef} className="relative">
      ...
      >
        {selected ? (
          ...
          <span className="font-body text-[14px] text-inkMute">Select a location…</span>
        )}
